...
RSpec.describe 'Statuses API', type: :request do
  ...
  after do
   ...
   it 'returns status records for given enrollments' do
      ...
      enrollment1 = json['data'].find_all{|status| status['attributes']['enrollmentId'] == @enrollment1.id.to_s}
      enrollment2 = json['data'].find_all{|status| status['attributes']['enrollmentId'] == @enrollment2.id.to_s}
   ...
   it 'returns status records for given students' do
      get "/api/statuses?studentIds=#{@student1.id},#{@student2.id}"
      ...
      student1 = json['data'].find_all{|status| status['attributes']['studentId'] == @student1.id.to_s}
      student2 = json['data'].find_all{|status| status['attributes']['studentId'] == @student2.id.to_s}

      expect(student1.size).to eq(3)
      expect(student2.size).to eq(3)

      expect(student1.first['relationships']).not_to be_empty
