...
class TinyPrivileges
  def initialize(values = nil)
    @privs = {
      ...
      view_note: false
    } # user can view any note
    @privs.update(values) unless values.nil?
  end

  def grant_all
    @privs.each { |key, _value| @privs[key] = true }
  end
  ...
  def[]=(priv, value)
    raise 'Unknown privilege type' unless @privs.key?(priv)
  ...
  def self.contract_child_object_privileges(user, theContract)
    ...
    p[:browse] =
      p[:view] = ((user_role >= Enrollment::ROLE_INSTRUCTOR) ||
