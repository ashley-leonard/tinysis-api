...
class GraduationPlan < ApplicationRecord
  include UnassignedCredits

  has_many :graduation_plan_mappings
  belongs_to :user

  validates_format_of :class_of, with: /2\d\d\d/, if: proc { |gp| !gp.class_of.nil? }, message: 'must be a valid 4-digit year'
  ...
  def map_credit_assignment(requirement, ca)
    unless (ca.user == user) || (ca.primary_parent == self)
      raise ArgumentError, 'Some kind of information leak'
    end

    ca.graduation_plan_mapping&.destroy
    mapping = graduation_plan_mappings.create(credit_assignment: ca, graduation_plan_requirement: requirement)
  end

  def privileges(user)
    self.user.privileges(user)
  end
end
