...
class Meeting < ApplicationRecord
  ...
  def create_participants
    ...
    q << "left join meeting_participants on meeting_participants.enrollment_id = enrollments.id and meeting_participants.meeting_id = #{id}"
    q << "where meeting_participants.id is null and enrollments.contract_id = #{contract_id} and enrollments.completion_status != #{Enrollment::COMPLETION_CANCELED}"

    missing = Enrollment.find_by_sql(q.join(' '))
    missing.each do |m|
      MeetingParticipant.create(meeting_id: id, enrollment_id: m.id, participation: 0)
    end
    missing
  end
