...
class Status < ApplicationRecord
  ...
  def self.make(month, statusable, user)
    unless month.day == 1
      raise ArgumentError, 'Invalid date value for status report'
    end

    ApplicationRecord.connection.execute("INSERT INTO statuses(month, creator_id, created_at, updated_at, statusable_id, statusable_type, met_fte_requirements)
        VALUES ('#{month.strftime('%Y-%m-%d')}', #{user.id}, NOW(), NOW(), #{statusable.id}, '#{statusable.class}', #{statusable.is_a?(Enrollment) ? 1 : 'NULL'})
        ON DUPLICATE KEY UPDATE updated_at = NOW()")
  end

  def privileges(user)
    statusable.privileges(user)
  end

  def unacceptable?
    (academic_status == Status::STATUS_UNACCEPTABLE) ||
      (attendance_status && attendance_status != STATUS_ACCEPTABLE) ||
      (statusable_type == 'User' && held_periodic_checkins == false) ||
      (statusable_type == 'Enrollment' && met_fte_requirements == false)
  end
  ...
  def self.contracts_months_missing(options = {})
    ...
    cond = [conditions.join(' and ')] + parameters unless conditions.empty?
    contracts = Contract.find(:all, conditions: cond, include: %i[term facilitator category], order: 'contracts.name')
