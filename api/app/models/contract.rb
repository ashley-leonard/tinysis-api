...
class Contract < ApplicationRecord
  ...
  def privileges(user)
    ...
    if user_role.nil?
      ...
      when User::PRIVILEGE_STUDENT
        ...
        if unsupervised && (user.id == creator.id)
    ...
    # contract
    if user_role >= Enrollment::ROLE_INSTRUCTOR || (unsupervised && user.id == creator_id)
      return p.grant_all
    end
  ...
  def <=>(contract)
    if category.sequence != contract.category.sequence
      return category.sequence <=> contract.category.sequence
    end
  ...
  def copy(params)
    ...
    attributes.each do |k, v|
      if %w[name timeslots learning_objectives category_id evaluation_methods competencies location instructional_materials].include?(k)
        attribs[k] = v
      end
    end
    if Contract.find(:first, conditions: ['name = ? and term_id = ? and facilitator_id = ?', params[:name], params[:term_id], params[:facilitator_id]])
      return nil
    end
  ...
  def self.catalog(options = {})
    ...
    if options[:facilitator_id]
      q << "AND (contracts.facilitator_id = #{options[:facilitator_id]})"
    end
    if options[:category_id]
      q << "AND (contracts.category_id = #{options[:category_id]})"
    end
  ...
  def unsupervised
    facilitator.nil? || facilitator.unassigned?
  end
  ...
  def attendance_stats
    ...
    mp_subtotals = MeetingParticipant.connection.select_rows(%{
  ...
  def attendance_hash(options = {})
    ...
    if options[:first] || options[:last]
      q << 'INNER JOIN meetings ON meetings.id = meeting_participants.meeting_id'
    end
    q << "WHERE enrollments.contract_id = #{id} AND enrollments.completion_status != #{Enrollment::COMPLETION_CANCELED}"
