...
class Credit < ApplicationRecord
  ...
  validates_uniqueness_of :course_id, if: proc { |cr| cr.course_id.present? && cr.course_id_changed? && (cr.course_id != '0') }, message: 'ID has already been used.'

  def course_type_string
    TYPE_NAMES[course_type].downcase
  end

  def self.admin_credit_report
    find_by_sql %{
  ...
  def self.options(transmittable = false)
    credits = if transmittable
                Credit.transmittable_credits
              else
                Credit.find(:all, order: 'course_type, course_name')
              end
  ...
  def course_id_string
    return '-' unless course_id.present? && (course_id != '0')

    Credit.format_course_id course_id
  end
  ...
  def destroy_credit
    credit_assignments.update_all ['credit_course_name = ?', course_name], 'credit_course_name IS NULL'
    credit_assignments.update_all ['credit_course_id = ?', course_name], 'credit_course_id IS NULL'

    if CreditAssignment.find(:first, conditions: ['(credit_id = ?) AND (enrollment_id IS NOT NULL) AND (credit_course_name IS NULL)', id])
      raise 'A credit without denormalized credit info is about to have its credit whacked'
    end
